...
Cincuenta y tres commits chicos en vez de veinte grandes. Tarda lo mismo, pero se lee mejor: cada diff entra en una pantalla y cada uno es un lugar donde frenar, mirar y decir «esto no».
...
**1 ·** `chore(app): crear el proyecto f4872` — [x]
...
**2 ·** `chore(app): conectar a Postgres con EF Core` — [x]
...
**3 ·** `chore(app): fijar la cultura es-AR` — [x]
...
**4 ·** `chore(app): traer la paleta y las tipografías del diseño` — [x]
...
**5 ·** `feat(datos): modelar el producto` — [x]
...
**6 ·** `feat(datos): modelar el ingrediente` — [x]
...
**7 ·** `feat(datos): modelar la base` — [x]
...
**8 ·** `feat(datos): relacionar el producto con sus ingredientes` — [x]
...
**9 ·** `feat(datos): relacionar la base con sus ingredientes` — [x]
...
**10 ·** `feat(datos): modelar el pedido` — [x]

Cliente, teléfono, cuándo se hizo, cuándo se entrega y el estado. Todavía sin ítems.

**11 ·** `feat(datos): guardar los ítems con su precio` — [x]
...
**12 ·** `feat(datos): guardar los ingredientes que se sacan` — [x]
...
**13 ·** `feat(datos): sembrar datos de prueba` — [x]
...
**14 ·** `feat(tienda): armar el armazón de la tienda` — [x]
...
**15 ·** `feat(tienda): disenar la pantalla de inicio` — [x]

El inicio existe para el que llega de cero: `48·72` no dice que es una pizzería, y el negocio funciona distinto a cualquier delivery. Tres credenciales, un recorrido de cuatro pasos con la fermentación como hueco, y el botón al final. Le cuesta un toque por semana al que ya es cliente, y por eso el botón ocupa el ancho entero.
→ *Verificar:* la pantalla mide 784 px de 798 y los íconos del recorrido quedan a 52 px parejos.

**16 ·** `feat(tienda): armar el inicio y mover la carta a /shop` — [x]

La pantalla en la app. **`/` pasa a ser el inicio y la carta se muda a `/shop`**, que es lo único que cambia respecto del commit 14.
→ *Verificar:* `/` muestra el inicio, «Armá tu pedido» lleva a `/shop`, y en 390 px entra sin scroll.

**17 ·** `feat(tienda): mostrar las pizzas y las focaccias` — [x]
...
**18 ·** `feat(tienda): mostrar las empanadas por pack` — [x]

Van aparte porque no tienen precio propio: se cobran por pack de 6 o de 12, de un solo gusto.

**19 ·** `feat(tienda): sumar los dibujos y agrandar el logo` — [x]

Los dibujos al costado del logo, los de las solapas, los de las credenciales del inicio y el logo un poco más grande. **Van juntos porque son la misma medición**: los tres primeros crecen la cabecera, y la cabecera es el presupuesto que gasta la regla de las siete pizzas sin scroll. Por eso también van después del 18: hasta que la carta no esté en pantalla no hay contra qué medir.
→ *Verificar:* la cabecera crece pero las siete pizzas siguen entrando sin scroll, y en 390 px ninguna solapa parte su texto.

**20 ·** `feat(tienda): contar y sumar productos al pedido` — [x]
...
**27 ·** `feat(tienda): mostrar la confirmación` — [x]

Qué pidió, cuánto sale y qué pasa ahora — que él escribe por WhatsApp.

**28 ·** `feat(tienda): mostrar la tienda cerrada` — [x]
...
**29 ·** `feat(panel): entrar al panel con una clave` — [x]
...
**30 ·** `feat(panel): armar la barra lateral y la cabecera` — [x]
...
**31 ·** `feat(panel): listar los pedidos` — [x]

La pantalla que va a mirar todos los días.

**32 ·** `feat(panel): ver el detalle de un pedido` — [x]
...
**33 ·** `feat(panel): avanzar y cancelar el estado del pedido` — [x]
...
**34 ·** `feat(app): avisar el pedido nuevo por Telegram` — [x]
...
**35 ·** `feat(panel): contar la producción del finde` — [x]

Los tres titulares: cuántos bollos, cuántas empanadas, cuántos pedidos.

**36 ·** `feat(panel): desglosar la producción por producto` — [x]
...
**39 ·** `feat(panel): dar de alta un producto` — [x]

El alta al pie de la lista, con el botón redondo.

**40 ·** `feat(panel): cargar la receta de un producto` — [x]
...
**41 ·** `feat(panel): elegir qué ingredientes se pueden sacar` — [x]

El interruptor por par — el que alimenta el commit 21.

**42 ·** `feat(panel): listar los ingredientes con su stock` — [x]

Con las cantidades legibles: 42 kg y no 42000 g.

**43 ·** `feat(panel): editar y dar de alta un ingrediente` — [x]

Nombre, unidad y la bandera de lo que no se compra.

**44 ·** `feat(panel): cargar la compra del ingrediente` — [x]

Cuánto trae y cuánto sale. De ahí sale el precio por gramo, que es lo que después usan los costos.

**45 ·** `feat(panel): cargar las bases por tanda` — [x]
...
**51 ·** `feat(panel): abrir y cerrar la tienda` — [x]

El interruptor que enciende el commit 28. Vive en la cabecera de Inicio, con el estado repetido en el pie de la barra lateral para que se vea desde cualquier pantalla.

**52 ·** `chore(app): preparar el deploy en Railway` — [x]
...
**Bootstrap se fue.** Vino con la plantilla de `dotnet new mvc` y no se usaba en ninguna pantalla: el diseño está medido al píxel y su reboot peleó con él desde el principio. Se fueron también jQuery y la validación no obtrusiva; `wwwroot` pasó de 9,9 MB a 385 KB.

**Las dos masas dejaron de ser una.** El modelo tenía una sola base, y la pizza y la focaccia no comparten bollo. Cuatro commits: separarlas, decir qué familia amasa cada una, sembrarlas también en producción y darle su masa a un producto nuevo.

**Costos mes por mes.** Los renglones 47 y 48 calculan lo que cuesta un producto, pero no había dónde mirar un mes cerrado contra lo que se cobró.

**Seis commits de tienda después del panel.** Tres credenciales más y el renglón del cuándo, el aviso de cerrada en el inicio, el botón que no se vacía mientras manda el pedido, el cartel que explica que los ingredientes se sacan, el acuse de recibo del contador y el total, y el trazo con el que entra cada pantalla.
