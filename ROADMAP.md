...
Cincuenta y dos commits chicos en vez de veinte grandes. Tarda lo mismo, pero se lee mejor: cada diff entra en una pantalla y cada uno es un lugar donde frenar, mirar y decir «esto no».
...
**19 ·** `feat(tienda): contar y sumar productos al pedido`
...
**26 ·** `feat(tienda): mostrar la confirmación`

Qué pidió, cuánto sale y qué pasa ahora — que él escribe por WhatsApp.

**27 ·** `feat(tienda): mostrar la tienda cerrada`
...
**28 ·** `feat(panel): entrar al panel con una clave`
...
**29 ·** `feat(panel): armar la barra lateral y la cabecera`
...
**30 ·** `feat(panel): listar los pedidos`

La pantalla que va a mirar todos los días.

**31 ·** `feat(panel): ver el detalle de un pedido`
...
**32 ·** `feat(panel): avanzar y cancelar el estado del pedido`
...
**33 ·** `feat(app): avisar el pedido nuevo por Telegram`
...
**34 ·** `feat(panel): contar la producción del finde`

Los tres titulares: cuántos bollos, cuántas empanadas, cuántos pedidos.

**35 ·** `feat(panel): desglosar la producción por producto`
...
**38 ·** `feat(panel): dar de alta un producto`

El alta al pie de la lista, con el botón redondo.

**39 ·** `feat(panel): cargar la receta de un producto`
...
**40 ·** `feat(panel): elegir qué ingredientes se pueden sacar`

El interruptor por par — el que alimenta el commit 20.

**41 ·** `feat(panel): listar los ingredientes con su stock`

Con las cantidades legibles: 42 kg y no 42000 g.

**42 ·** `feat(panel): editar y dar de alta un ingrediente`

Nombre, unidad y la bandera de lo que no se compra.

**43 ·** `feat(panel): cargar la compra del ingrediente`

Cuánto trae y cuánto sale. De ahí sale el precio por gramo, que es lo que después usan los costos.

**44 ·** `feat(panel): cargar las bases por tanda`
...
**50 ·** `feat(panel): abrir y cerrar la tienda`

El interruptor que enciende el commit 27. Vive en la cabecera de Inicio, con el estado repetido en el pie de la barra lateral para que se vea desde cualquier pantalla.

**51 ·** `chore(app): preparar el deploy en Railway`
...
**52 ·** `docs(app): escribir el README`
